...
export default class MessageHandler {
  ...
  handleMessage = async (M: ISimplifiedMessage): Promise<void> => {
    ...
    if (state)
      return void M.reply(
        `✖ This command is disabled by carter.${
    ...
    if (
      ...
      return void M.reply(`Only carter & his MODS are allowed to use this command.`);
    }
  ...
  handleState = async (): Promise<void> => {
    const text = `©FUTURE-BOTTO 2022`;
    await this.client.sendMessage(
      "120363022304789641@g.us",
      text,
      MessageType.text
    );
  };
